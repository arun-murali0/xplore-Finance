...
export default function Hero() {
	return (
		<Wrapper className="flex flex-col items-center justify-between h-full max-md:m-auto py-16 max-sm:px-1">
			<div className="flex justify-between items-center mt-20 max-w-3xl  md:w-full max-sm:mt-8 max-sm:flex max-sm:justify-center">
				<div>
					...
					<p className="text-base text-muted-foreground  mt-6 text-center max-sm:text-sm max-sm:ms-2 max-sm:mt-3 max-sm:text-start max-sm:leading-5">
						Master the art of stock market investing through
						our comprehensive, beginner-friendly courses.
						Unlock the secrets to smart trading and
						financial growth.
					</p>
					<div className="hidden md:flex relative items-center justify-center mt-8 w-full max-sm:block max-sm:px-3">
						<div className="flex items-center justify-center w-max rounded-full px-2 py-1 md:py-2 gap-2 md:gap-8 shadow-3xl  select-none max-sm:text-xs max-sm:flex-col">
							<p className="text-primary text-sm text-center md:text-base font-medium pl-4 pr-4 lg:pr-0 max-sm:text-xs max-sm:hidden">
          ...
          <video autoPlay  width={500} height={500} loop>
